...
#ifndef PART_DIRECT_AGGREGATED_RECVREQ_H
#define PART_DIRECT_AGGREGATED_RECVREQ_H

#include "ompi/mca/part/direct_aggregated/part_direct_aggregated_request.h"
#include "ompi/mca/part/base/part_base_precvreq.h"

struct mca_part_direct_aggregated_precv_request_t {
    mca_part_direct_aggregated_request_t req_base;
};
typedef struct mca_part_direct_aggregated_precv_request_t mca_part_direct_aggregated_precv_request_t;
OBJ_CLASS_DECLARATION(mca_part_direct_aggregated_precv_request_t);
 ...
 */
#define MCA_PART_DIRECT_AGGREGATED_PRECV_REQUEST_ALLOC(precvreq)                           \
do {                                                                         \
    precvreq = (mca_part_direct_aggregated_precv_request_t*)                               \
      opal_free_list_get (&mca_part_direct_aggregated_precv_requests);                    \
    precvreq->req_base.req_type = MCA_PART_DIRECT_AGGREGATED_REQUEST_PRECV;            \
 } while (0)
 ...
 * @param direct_aggregatedent (IN)    Is this a ersistent request.
 */
#define MCA_PART_DIRECT_AGGREGATED_PRECV_REQUEST_INIT( request,                     \
                                         ompi_proc,                   \
                                         comm,                        \
                                         tag,                         \
                                         src,                         \
 ...
 */
#define MCA_PART_DIRECT_AGGREGATED_PRECV_REQUEST_RETURN(recvreq)                      \
{                                                                       \
    ...
    opal_free_list_return ( &mca_part_direct_aggregated_precv_requests,               \
