...
#include "ompi/mca/part/base/aggregation_schemes/aggregation_scheme_rb_tree.h"
...
#define MCA_PART_DIRECT_AGGREGATED_PSEND_REQUEST_ALLOC(sendreq, comm, dst,          \
                                           ompi_proc)                 \
do {                                                                  \
    sendreq = (mca_part_direct_aggregated_psend_request_t*)                         \
        opal_free_list_wait (&mca_part_direct_aggregated_psend_requests);          \
    sendreq->req_base.req_type = MCA_PART_DIRECT_AGGREGATED_REQUEST_PSEND;          \
} while(0)

#define MCA_PART_DIRECT_AGGREGATED_PSEND_REQUEST_INIT( req_send,                    \
                                         ompi_proc,                   \
                                         comm,                        \
                                         tag,                         \
                                         dst,                         \
 ...
 */
#define MCA_PART_DIRECT_AGGREGATED_PSEND_REQUEST_RETURN(sendreq)                      \
    {                                                                   \
        ...
        opal_free_list_return ( &mca_part_direct_aggregated_psend_requests,            \
