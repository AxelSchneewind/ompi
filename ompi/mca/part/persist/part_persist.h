...
mca_part_persist_free_req(struct mca_part_persist_request_t* req)
{
    ...
    if (MCA_PART_PERSIST_REQUEST_PSEND == req->req_type) {
        mca_part_persist_psend_request_t *sendreq = (mca_part_persist_psend_request_t *) req;
        aggregation_scheme_regular_free(&sendreq->aggregation_state);
    }
                        ...
                        ompi_request_t** request)
{
    ...
    aggregation_scheme_regular_init(&sendreq->aggregation_state, req->real_parts, factor, remaining_partitions);
...
mca_part_persist_start(size_t count, ompi_request_t** requests)
{
    ...
    for(i = 0; i < _count && OMPI_SUCCESS == err; i++) {
        ...
        if (MCA_PART_PERSIST_REQUEST_PSEND == req->req_type) {
            mca_part_persist_psend_request_t *sendreq = (mca_part_persist_psend_request_t *)(req);
            aggregation_scheme_regular_reset(&sendreq->aggregation_state);
        }
                    ...
                    ompi_request_t* request)
{
    ...
    for(i = min_part; i <= max_part && OMPI_SUCCESS == err; i++) {
        aggregation_scheme_regular_pready(&sendreq->aggregation_state, i, &internal_part_ready);
