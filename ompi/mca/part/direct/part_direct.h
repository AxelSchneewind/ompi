                        ...
                        ompi_request_t** request)
{
    ...
    part_persist_aggregate_interval_tree_init(&sendreq->aggregation_state, factor);
...
mca_part_direct_start(size_t count, ompi_request_t** requests)
{
    ...
    for(i = 0; i < _count && OMPI_SUCCESS == err; i++) {
        ...
        if(MCA_PART_DIRECT_REQUEST_PSEND == req->req_type) {
            ...
            part_persist_aggregate_interval_tree_reset(&sendreq->aggregation_state);
        } else {
            req->done_count = 0;
                    ...
                    ompi_request_t* request)
{
    ...
    int left, right;
    int extracted = part_persist_aggregate_interval_tree_pready_range(&sendreq->aggregation_state, min_part, max_part, &left, &right);

    if (extracted) // enqueue interval
    {
