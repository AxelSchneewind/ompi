...
#ifndef PART_DIRECT_ORIGINAL_H
#define PART_DIRECT_ORIGINAL_H
...
#include "ompi/mca/part/direct_original/part_direct_original_recvreq.h"
#include "ompi/mca/part/direct_original/part_direct_original_sendreq.h"
#include "ompi/message/message.h"
#include "ompi/mca/pml/pml.h"
BEGIN_C_DECLS

typedef struct mca_part_direct_original_list_t {
    opal_list_item_t        super;
    mca_part_direct_original_request_t *item;
} mca_part_direct_original_list_t;

OPAL_DECLSPEC OBJ_CLASS_DECLARATION(mca_part_direct_original_list_t);


struct ompi_part_direct_original_t {
...
typedef struct ompi_part_direct_original_t ompi_part_direct_original_t;
extern ompi_part_direct_original_t ompi_part_direct_original;


/**
 * This is a helper function that frees a request. This requires ompi_part_direct_original.lock be held before calling.
 */
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_free_req(struct mca_part_direct_original_request_t* req)
{
    ...
    if( MCA_PART_DIRECT_ORIGINAL_REQUEST_PRECV == req->req_type ) {
        MCA_PART_DIRECT_ORIGINAL_PRECV_REQUEST_RETURN(req);
    } else {
        MCA_PART_DIRECT_ORIGINAL_PSEND_REQUEST_RETURN(req);
    }
    return err;
}


__opal_attribute_always_inline__ static inline void mca_part_direct_original_init_lists(void)
{
    opal_free_list_init (&mca_part_direct_original_precv_requests,
                         sizeof(mca_part_direct_original_precv_request_t),
                         opal_cache_line_size,
                         OBJ_CLASS(mca_part_direct_original_precv_request_t),
                         0,opal_cache_line_size,
                         ompi_part_direct_original.free_list_num,
                         ompi_part_direct_original.free_list_max,
                         ompi_part_direct_original.free_list_inc,
                         NULL, 0, NULL, NULL, NULL);
    opal_free_list_init (&mca_part_direct_original_psend_requests,
                         sizeof(mca_part_direct_original_psend_request_t),
                         opal_cache_line_size,
                         OBJ_CLASS(mca_part_direct_original_psend_request_t),
                         0,opal_cache_line_size,
                         ompi_part_direct_original.free_list_num,
                         ompi_part_direct_original.free_list_max,
                         ompi_part_direct_original.free_list_inc,
                         NULL, 0, NULL, NULL, NULL);
     ompi_part_direct_original.progress_list = OBJ_NEW(opal_list_t);
}

__opal_attribute_always_inline__ static inline void
mca_part_direct_original_complete(struct mca_part_direct_original_request_t* request)
{
    if(MCA_PART_DIRECT_ORIGINAL_REQUEST_PRECV == request->req_type) {
...
/**
 * mca_part_direct_original_progress is the progress function that will be registered. It handles 
 ...
 */
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_progress(void)
{
    mca_part_direct_original_list_t *current;
    int err;
    size_t i;

    /* prevent re-entry, */
    int block_entry = opal_atomic_add_fetch_32(&(ompi_part_direct_original.block_entry), 1);
    if(1 < block_entry)
    {
        block_entry = opal_atomic_add_fetch_32(&(ompi_part_direct_original.block_entry), -1);
        return OMPI_SUCCESS;
    }

    OPAL_THREAD_LOCK(&ompi_part_direct_original.lock);
 
    mca_part_direct_original_request_t* to_delete = NULL;

    OPAL_LIST_FOREACH(current, ompi_part_direct_original.progress_list, mca_part_direct_original_list_t) {
        mca_part_direct_original_request_t *req = (mca_part_direct_original_request_t *) current->item;
    ...
    OPAL_THREAD_UNLOCK(&ompi_part_direct_original.lock);
    block_entry = opal_atomic_add_fetch_32(&(ompi_part_direct_original.block_entry), -1);
    if(to_delete) {
        err =  mca_part_direct_original_free_req(to_delete);
        if (OMPI_SUCCESS != err) {
            return OMPI_ERROR;
        }
    }
...
__opal_attribute_always_inline__ static inline void
mca_part_direct_original_create_partition_communicator(MPI_Comm comm,
                                   int rank_count,
                                   const int ranks[],
                                   MPI_Comm* new_comm)
{
...
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_precv_init(void *buf,
                        size_t parts, 
                        size_t count,
                        ...
                        struct ompi_request_t **request)
{
    ...
    size_t dt_size_;
    int dt_size;
    mca_part_direct_original_list_t* new_progress_elem = NULL;

    mca_part_direct_original_precv_request_t *recvreq;


    /* Allocate a new request */
    MCA_PART_DIRECT_ORIGINAL_PRECV_REQUEST_ALLOC(recvreq);
    if (OPAL_UNLIKELY(NULL == recvreq)) return OMPI_ERR_OUT_OF_RESOURCE;

    MCA_PART_DIRECT_ORIGINAL_PRECV_REQUEST_INIT(recvreq, ompi_proc, comm, tag, src,
                                     datatype, buf, parts, count, flags);

    mca_part_direct_original_request_t *req = (mca_part_direct_original_request_t *) recvreq;
    ...
    new_progress_elem = OBJ_NEW(mca_part_direct_original_list_t);
    new_progress_elem->item = req;
    req->progress_elem = new_progress_elem; 
    OPAL_THREAD_LOCK(&ompi_part_direct_original.lock);
    opal_list_append(ompi_part_direct_original.progress_list, (opal_list_item_t*)new_progress_elem);
    OPAL_THREAD_UNLOCK(&ompi_part_direct_original.lock);
...
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_psend_init(const void* buf,
                        size_t parts,
                        size_t count,
                        ompi_datatype_t* datatype,
                        int dst,
                        ...
                        ompi_request_t** request)
{
    ...
    int dt_size;
    mca_part_direct_original_list_t* new_progress_elem = NULL;
    mca_part_direct_original_psend_request_t *sendreq;

    /* Create new request object */
    MCA_PART_DIRECT_ORIGINAL_PSEND_REQUEST_ALLOC(sendreq, comm, dst, ompi_proc);
    if (OPAL_UNLIKELY(NULL == sendreq)) return OMPI_ERR_OUT_OF_RESOURCE;
    MCA_PART_DIRECT_ORIGINAL_PSEND_REQUEST_INIT(sendreq, ompi_proc, comm, tag, dst,
                                    datatype, buf, parts, count, flags);
    mca_part_direct_original_request_t *req = (mca_part_direct_original_request_t *) sendreq;
...
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_start(size_t count, ompi_request_t** requests)
{
    int err = OMPI_SUCCESS;
    size_t _count = count;
    size_t i;
...
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_pready(size_t min_part,
                    size_t max_part,
                    ompi_request_t* request)
{
    int err = OMPI_SUCCESS;
    ...
    mca_part_direct_original_request_t *req = (mca_part_direct_original_request_t *)(request);
...
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_parrived(size_t min_part,
                      size_t max_part,
                      int* flag, 
                      ompi_request_t* request)
{
...
/**
 * mca_part_direct_original_free marks an entry as free called and sets the request to 
 * MPI_REQUEST_NULL. Note: requests get freed in the progress engine. 
 */
__opal_attribute_always_inline__ static inline int
mca_part_direct_original_free(ompi_request_t** request)
{
    mca_part_direct_original_request_t* req = *(mca_part_direct_original_request_t**)request;
...
#endif  /* PART_DIRECT_ORIGINAL_H_HAS_BEEN_INCLUDED */
